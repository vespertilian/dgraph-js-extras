...
describe("delete-json-now", () => {
  ...
  it("lets you delete multiple nodes (2x addresses) and the links that those noes had to another node (user has addresses - delete two links pointing to addresses", async() => {
    ...
    // address2 is george
    const {a2} = await xQueryNow({query: queryAddressByUid('a2', address2Uid)}, dgraphClient);
    ...
    // we can no longer query for the old address
    const {a3} = await xQueryNow({query: queryAddressByUid('a3', address1Uid)}, dgraphClient);
    expect(a3[0].street).toBeUndefined();
    expect(a3[0].postcode).toBeUndefined();

    const {a4} = await xQueryNow({query: queryAddressByUid('a4', address2Uid)}, dgraphClient);
